...
import array as arr
import ROOT
from ROOT import TCanvas, TH1F, TH2F, TFile, TF1, TPad, TGraphErrors, TLine, TLegend, TLatex
...
wMapdict = {"egi": [0.1, 0.175, 0, 0.175, 0.1, 0.11667, 0.11667, 0.11667, 0, 0, 0.1],\
            "nominal": [1, 1, 0, 1, 1, 1, 1, 1, 0, 0, 1],\
            "equal": [1, 0.5, 0, 0.5, 1, 0.3333, 0.3333, 0.3333, 0, 0, 1]}
...
ArchDate = {"Naples": 2017.25, "Rome": 2019.5, "Milan": 2021.0, "Broadwell": 2016.0, "Haswell": 2014.5, "NehalemEP": 2009.0, \
            "CascadeLake": 2019.25, "Skylake": 2017.5, "IceLake": 2021.25, "IvyBridgeEP": 2013.5}
...
for i in range(len(score)):
    ...
    for j in range(len(HS64)):
        if cpu == HS64[j][0]:
            ...
            date = ArchDate[CPUArch[cpu]]
            if "Intel" in cpu and "HT1" in cpu:
                x1.append(date); y1.append(ratio); cpu1.append(cpu)
            if "Intel" in cpu and "HT2" in cpu:
                x2.append(date); y2.append(ratio); cpu2.append(cpu)
            if "AMD" in cpu   and "HT1" in cpu:
                x3.append(date); y3.append(ratio); cpu3.append(cpu)
            if "AMD" in cpu   and "HT2" in cpu:
                x4.append(date); y4.append(ratio); cpu4.append(cpu)
...
if h1: del h1
h1 = TH2F("h1","",56,2008,2022,50,-0.5,0.5)
...
h1.GetXaxis().SetTitle( "Year CPU Released" );
